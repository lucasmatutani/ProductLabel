...
use Lof\ProductLabel\Model\AbstractLabel;
use Lof\ProductLabel\Model\ResourceModel\Index;
...
class IndexBuilder
{
    ...
    public function reindexByProductIds(array $ids)
    {
        ...
        try {
            ...
            throw new LocalizedException(
                __("Lof label indexing failed. See details in exception log.")
            );
        }
    }
    ...
    private function doReindexByLabelIds($ids)
    {
        ...
        foreach ($this->getLabelCollection($ids) as $label) {
            ...
            $this->cacheContext->registerEntities(AbstractLabel::CACHE_TAG, [$label->getId()]);
     ...
     * @return $this
     */
    private function reindexByLabelAndProductIds(\Lof\ProductLabel\Model\Label $label, $ids = null)
    {
        if (!$ids) {
            return $this;
        }

        $test = $this->prepareData($label, $ids);
