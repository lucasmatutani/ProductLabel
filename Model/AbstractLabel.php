 ...
 */
class AbstractLabel extends \Magento\Framework\Model\AbstractModel {
	/**
	 * Label cache tag
	 */
	const CACHE_TAG = 'lof_productlabel_label';

	const TYPE_GIFTCARD = 'giftcard';

	public $_cacheTag = 'lof_productlabel_label';
	 ...
	 * @param \Lof\ProductLabel\Model\GiftCard\Model\Product\ReadHandler   $readHandler
	...
	public function __construct(
		...
		\Lof\ProductLabel\Model\GiftCard\Model\Product\ReadHandler $readHandler,
	...
	public function init( Product $product, $mode = null, $parent = null )
	{
		...
		if ( $mode ) {
			$this->setMode( $mode == 'category' ? 'cat' : 'product' );
		} else {
			$this->setMode( 'cat' );
		}
	}
	...
	public function getLabelMatchingProductIds( $ids = null )
	{
		if ( $this->getData( 'cond_serialize' ) !== '' ) {
			/** @var \Lof\ProductLabel\Model\Rule $ruleModel */
	...
	{
		parent::_construct();
		$this->_init( \Lof\ProductLabel\Model\ResourceModel\Label::class );
